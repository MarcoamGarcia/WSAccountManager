...
(function($){
        ...
        replacement = $.each(
	        {
	            css:
                    ...
                    hide: function(){
                        var elem = $(this);
                        var display_style = elem.css('display');

                        if (display_style !== "none" && !elem.data('old_display_style')) {
                            elem.data('old_display_style', display_style);
                        }
                        var ori_method = original.hide.apply(this, arguments);
                        elem.css('display', 'none', true);
                        return ori_method;
                    },
                    show: function(){
                        var elem = $(this);
                        var ori_method = original.show.apply(this, arguments);
                        var old_display = elem.data('old_display_style');
                        if(old_display) {
                            // changed from 'inline' to 'old_display' so it works with div and other 'block' elements.
                            elem.css('display', old_display, true);
                        } else {
                            elem.css('display', '', true);
                        }
                        return ori_method;
                    }
	...
	$.fn.important = function(method){
        ...
        if (typeof method === 'undefined' || typeof method === 'boolean'){
            ...
            if (!nodeName){
                nodeName = elem.get(0).nodeName.toLowerCase();
