  ...
  $.fn.joyride = function(options) {
    ...
    return this.each(function() {
      ...
      var tipLayout = function(tipClass, index, buttonText, self) {
        ...
        if (settings.inline) {
          parent_body.find(tipTemplate(tipClass, index, buttonText, self)).insertAfter('#' + $(self).data('id'));
        } else {
      ...
      showNextTip = function() {
        ...
        $.each((parent_body.find(tipContent[count]).data('options') || ':').split(';'),
          function (i, s) {
            var p = s.split(':');
            if (p.length == 2) {
        ...
        if (count < tipContent.length) {
          if (settings.tipAnimation == "pop") {
            ...
            if (settings.timer > 0) {
              currentTip.show().children('.joyride-timer-indicator-wrap')
                ...
                .animate({width: parent_body.find('.joyride-timer-indicator-wrap')
          ...
          if ($(parent_window).scrollLeft() + $(parent_window).width() < left + currentTip.width()) {
      ...
      $(parent_window).resize(function () {
        ...
        if (Modernizr.mq('only screen and (max-width: 769px)')) {
          if (settings.tipLocation == "bottom") {
              parent_body.find('#joyRidePopup' + prevCount).offset({
              top: (currentTipPosition.top + currentParentHeight + nubHeight),
              left: 0
            });
            parent_body.find('#joyRidePopup' + prevCount).children('.joyride-nub').addClass('top').removeClass('bottom').css({ left: (currentTipPosition.left - bodyOffset.left) });
            ...
            if (currentTipPosition.top <= currentTipHeight) {
              ...
              parent_body.find('#joyRidePopup' + prevCount).children('.joyride-nub').addClass('bottom').removeClass('top').css({ left: (currentTipPosition.left - bodyOffset.left) });
            }
          }
        } else {
          if (settings.tipLocation == "bottom") {
            ...
            parent_body.find('#joyRidePopup' + prevCount).children('.joyride-nub').addClass('top').removeClass('bottom').css({ left: '' });
          } else if (settings.tipLocation == "top") {
            if (currentTipPosition.top <= currentTipHeight) {
              parent_body.find('#joyRidePopup' + prevCount).offset({
                top: (currentTipPosition.top + nubHeight + currentParentHeight),
              ...
              parent_body.find('#joyRidePopup' + prevCount).children('.joyride-nub').addClass('bottom').removeClass('top').css({ left: '' });
            }
          }
        }
      });
