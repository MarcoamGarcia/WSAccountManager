...
(function($) {
  $.fn.joyride = function(options) {
    ...
    return this.each(function() {
      ...
      if (parent_body.find(options.tipContent).length === 0) return;

      parent_body.find(options.tipContent).hide();


      var bodyOffset;
      if(options.tipContainer == 'body') {
          bodyOffset = parent_body.children('*').first().position();
      } else {
          bodyOffset = parent_body.find(options.tipContainer).children('*').first().position();
      }
      var tipContent = parent_body.find(options.tipContent + ' li'),
      count = skipCount = 0,
      prevCount = -1,
      ...
      tipTemplate = function(tipClass, index, buttonText, self) { return '<div class="joyride-tip-guide ' + tipClass + '" id="joyRidePopup' + index + '"><span class="joyride-nub"></span>' + parent_body.find(self).html() + buttonText + '<a href="#close" class="joyride-close-tip">X</a>' + timerIndicatorInstance + '</div>'; },
      tipLayout = function(tipClass, index, buttonText, self) {
        ...
        if (settings.inline) {
          parent_body.find(tipTemplate(tipClass, index, buttonText, self)).insertAfter('#' + parent_body.find(self).data('id'));
        } else {
          var tipTemp = tipTemplate(tipClass, index, buttonText, self);
          if(options.tipContainer == 'body') {
              parent_body.append(tipTemp);
          } else {
              parent_body.find(options.tipContainer).append(tipTemp);
          }
      ...
      tipContent.each(function(index) {
        ...
        parent_body.find('#joyRidePopup' + index).hide();
      });
    }

      showNextTip = function() {
        var parentElementID = parent_body.find(tipContent[count]).data('id'),
        parentElement = parent_body.find('#' + parentElementID);

        while (parentElement.offset() === null) {
          count++;
          skipCount++;
          prevCount++;
          parentElementID = parent_body.find(tipContent[count]).data('id'),
          parentElement = parent_body.find('#' + parentElementID);

          if (parent_body.find(tipContent).length < count)
            break;
        }
        var windowHalf = Math.ceil($(parent_window).height() / 2),
        currentTip = parent_body.find('#joyRidePopup' + count),
        ...
        nubHeight = Math.ceil(parent_body.find('.joyride-nub').outerHeight() / 2),
        tipOffset = 0;
        ...
        if (count < tipContent.length) {
          if (settings.tipAnimation == "pop") {
            parent_body.find('.joyride-timer-indicator').width(0);
            if (settings.timer > 0) {
              currentTip.show().children('.joyride-timer-indicator-wrap').children('.joyride-timer-indicator').animate({width: parent_body.find('.joyride-timer-indicator-wrap').width()}, settings.timer);
            } else {
              currentTip.show();
            }
            ...
            parent_body.find('.joyride-timer-indicator').width(0);
            if (settings.timer > 0) {
              currentTip.fadeIn(settings.tipAnimationFadeSpeed).children('.joyride-timer-indicator-wrap').children('.joyride-timer-indicator').animate({width: parent_body.find('.joyride-timer-indicator-wrap').width()}, settings.timer);
            } else {
          ...
          if (count > 0) {
            ...
            if (settings.tipAnimation == "pop") {
              parent_body.find('#joyRidePopup' + hideCount).hide();
            } else if (settings.tipAnimation == "fade") {
              parent_body.find('#joyRidePopup' + hideCount).fadeOut(settings.tipAnimationFadeSpeed);
            }
          }
        ...
        // Hide the last tip when clicked
          ...
          if (settings.tipAnimation == "pop") {
            parent_body.find('#joyRidePopup' + hideCount).fadeTo(0, 0);
          } else if (settings.tipAnimation == "fade") {
            parent_body.find('#joyRidePopup' + hideCount).fadeTo(settings.tipAnimationFadeSpeed, 0);
          }
        }
        count++;
      ...
      if (!settings.inline || !settings.cookieMonster || !$.cookie(settings.cookieName)) {
        $(parent_window).resize(function() {
          var parentElementID = parent_body.find(tipContent[prevCount]).data('id'),
          currentTipPosition = parent_body.find('#' + parentElementID).offset(),
          currentParentHeight = parent_body.find('#' + parentElementID).outerHeight(),
          currentTipHeight = parent_body.find('#joyRidePopup' + prevCount).outerHeight(),
          nubHeight = Math.ceil(parent_body.find('.joyride-nub').outerHeight() / 2);
          if (settings.tipLocation == "bottom") {
            parent_body.find('#joyRidePopup' + prevCount).offset({top: (currentTipPosition.top + currentParentHeight + nubHeight), left: currentTipPosition.left});
          } else if (settings.tipLocation == "top") {
            if (currentTipPosition.top <= currentTipHeight) {
              parent_body.find('#joyRidePopup' + prevCount).offset({top: (currentTipPosition.top + nubHeight + currentParentHeight), left: currentTipPosition.left});
            } else {
              parent_body.find('#joyRidePopup' + prevCount).offset({top: ((currentTipPosition.top) - (currentTipHeight  + nubHeight)), left: currentTipPosition.left});
            }
          }
        });
      }
      ...
      parent_body.find('.joyride-close-tip').click(function(e) {
      ...
      // When the next button is clicked, show the next tip, only when cookie isn't present
        parent_body.find('.joyride-next-tip').click(function(e) {
          e.preventDefault();
          if (count >= tipContent.length) {
            endTip(e, interval_id, settings.cookieMonster, this);
          }
